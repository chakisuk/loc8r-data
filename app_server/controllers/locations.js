...
const homelist = (req, res) => {
  ...
  const requestOptions = {
    ...
    json: {},
    qs: {
      lng:127.2652937148756,
      lat:37.011314314751154,
      maxDistance: 2000000
    }
  };
    request(
...
const renderHompage = function(req, res, responseBody) {
  ...
  res.render(`location-list`, {
    title: 'Loc8r - find a plcae to work with wifi',
    pageHeader: {
      title: 'Loc8r',
      strapline: 'Find plcaes to work with wifi near you!'
    },
    sidebar: "Looking for wifi and a seat? Loc8r helps you find places \
    to work when out and about. Perhaps with coffr, cake or a pint? \
...
const showError = (req, res, status) => {
  ...
  if (status === 404) {
    ...
    title = `${status}, something's gont wrong`;
